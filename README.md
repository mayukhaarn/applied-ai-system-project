...
- `ai_system/` — core pipeline modules for retrieval, agent logic, and evaluation.
- `tests/` — evaluation harness and automated checks.
- `docs/` — model card, ethical reflection, and design documentation.
- `index.html` — GitHub Pages demo UI.
- `main.py` — pipeline entrypoint for local execution.
- `fi_*` scripts — existing fi.co integration modules for scraping, grading, sheets, and email.
...
- **Eval harness results**: `eval_harness.py` ran 5 predefined queries and reported `5 out of 5 tests passed`. Average confidence across the evaluated outputs was `0.78`.
...
This project now includes a tone toggle for the agent to demonstrate specialized model behavior. The system supports `firm`, `calm`, and `professional` delivery styles, and compares each styled response against a neutral baseline.

- `firm`: direct and decisive phrasing
- `calm`: gentle, reassuring wording
- `professional`: polished and structured business language

The pipeline now returns both a baseline and tone-specific answer, along with a measurable style difference flag:

```python
result = run("Summarize the Founder Institute mission.", tone="calm")
print(result["style_comparison"])
```

This proves the output differs from the baseline by using constrained style patterns and synthetic few-shot examples inside the agent.

## Stretch Feature Recommendation

The easiest stretch feature to implement given the current architecture is a Retrieval-Augmented Generation (RAG) enhancement. The repository already separates retrieval from generation, so adding a real content lookup layer in `ai_system/retriever.py` is a natural extension.

### Suggested implementation

Add a lightweight local knowledge base to the retriever and return matching context for queries.

```python
# ai_system/retriever.py
class Retriever:
    def __init__(self, sources: list[str] | None = None):
        self.sources = sources or ["default_corpus"]
        self.knowledge_base = {
            "founder institute": "Founder Institute is a global startup accelerator that helps founders launch fundable companies.",
            "homework grading": "Homework grading uses consistency, completeness, and originality metrics to score submissions.",
        }

    def retrieve(self, query: str) -> dict[str, Any]:
        query_lower = query.lower()
        matched = [text for key, text in self.knowledge_base.items() if key in query_lower]
        context_text = matched[0] if matched else "General accelerator coaching context."
        return {
            "query": query,
...
This adds real retrieval behavior without requiring external APIs, and it lays the groundwork for a future vector-store or document search layer.
...
- Diagram files and visual assets are stored in the `/assets` folder.
